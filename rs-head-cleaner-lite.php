...
Plugin URI: https://www.redsandmarketing.com/plugins/rs-head-cleaner-lite/
Description: This plugin combines/minifies/caches CSS & JavaScript files, removes junk code from the document HEAD & HTTP headers, hides the Generator/WordPress Version number, removes version numbers from CSS and JS links, minifies HTML, and fixes the "Read more" link so it displays the entire post. The plugin cleans up a number of issues, replacing multiple plugins, improving speed, efficiency, security, SEO, and user experience.
Author: Scott Allen
Version: 1.4.3.8
 ...
 *	- Disable Emoji Detection			- OFF BY DEFAULT - ENABLE USING wp-config.php constants - (Optional) Disables the emoji detection for older browsers and removes significant code bloat from the head and other places. Based on code from "Disable Emojis" plugin. ( https://wordpress.org/plugins/disable-emojis/ )
 ...
 *	- Minify HTML						- OFF BY DEFAULT - ENABLE USING wp-config.php constants - (Optional) Removes comments from HTML and minifies HTML code
 *	- JavaScript to Footer 				- OFF BY DEFAULT - ENABLE USING wp-config.php constants - (Optional) For Speed in page loading - Part of the Combine, Minify, and Cache
